...
if [ "$1" != "--running" ]; then
    MASTER_LOG="log/freeway_adaptive_policy_seed234_$(date '+%y%m%d%H%M').log"
    mkdir -p log
    nohup "$0" --running > "$MASTER_LOG" 2>&1 &
    PID=$!
    echo "=========================================="
    echo "Freeway: Adaptive Policy (seed 2,3,4)"
    echo "  PID: $PID"
    echo "  Master log: $MASTER_LOG"
    echo "=========================================="
    echo ""
    echo "Experiments (3 total):"
    echo "  Adaptive Policy (dormant + adaptive_policy + dist:normal): seed 2,3,4"
    echo ""
...
echo "=========================================="
echo "Starting Freeway: Adaptive Policy experiments (seed 2,3,4)"
...
###############################################################################
# Adaptive Policy 実験 (dormant + adaptive_policy + dist:normal) - seed 2,3,4
###############################################################################

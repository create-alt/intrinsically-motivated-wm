...
if [ "$1" != "--running" ]; then
    MASTER_LOG="log/mspacman_ema_policy_shifting_$(date '+%y%m%d%H%M').log"
    mkdir -p log
    nohup "$0" --running > "$MASTER_LOG" 2>&1 &
    PID=$!
    echo "=========================================="
    echo "MsPacman: EMA-Based Policy Shifting (5 seeds)"
    echo "  PID: $PID"
...
echo "=========================================="
echo "Starting MsPacman: EMA-Based Policy Shifting experiments"

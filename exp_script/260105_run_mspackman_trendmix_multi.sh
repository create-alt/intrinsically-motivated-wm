...
if [ "$1" != "--running" ]; then
    ...
    PID=$!
    echo "=========================================="
    echo "All 3 experiments started in background"
    echo "  PID: $PID"
...
###############################################################################
# 実験7: fast=0.01, slow=0.001
###############################################################################
EXP_NAME="exp07_fast001_slow0001"
TIME_STR=$(date '+%y%m%d%H%M')
LOG_DIR="log/${TIME_STR}_dreamerV3_atari100k_ms_pacman_trendmix_${EXP_NAME}"
mkdir -p ${LOG_DIR}
echo "=========================================="
echo "[Experiment 7] ${EXP_NAME}"
echo "Log directory: ${LOG_DIR}"
echo "Started at: $(date)"
echo "=========================================="

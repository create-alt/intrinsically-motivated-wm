...
if [ "$1" != "--running" ]; then
    MASTER_LOG="log/trendmix_exp07_4games_$(date '+%y%m%d%H%M').log"
    mkdir -p log
    nohup "$0" --running > "$MASTER_LOG" 2>&1 &
    PID=$!
    echo "=========================================="
    echo "TrendMixture exp07: 4 experiments started in background"
    echo "  PID: $PID"
...
echo "=========================================="
echo "Starting TrendMixture exp07 experiments (3 games)"

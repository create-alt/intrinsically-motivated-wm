...
if [ "$1" != "--running" ]; then
    MASTER_LOG="log/bank_heist_baseline_$(date '+%y%m%d%H%M').log"
    mkdir -p log
    nohup "$0" --running > "$MASTER_LOG" 2>&1 &
    PID=$!
    echo "=========================================="
    echo "Bank Heist: Baseline (5 seeds)"
    echo "  PID: $PID"
...
echo "=========================================="
echo "Starting Bank Heist: Baseline experiments"
...
echo "Total: 5 experiments (5 baseline)"
echo "=========================================="

...
if [ "$1" != "--running" ]; then
    MASTER_LOG="log/mspacman_recency_exp01_$(date '+%y%m%d%H%M').log"
    mkdir -p log
    nohup "$0" --running > "$MASTER_LOG" 2>&1 &
    PID=$!
    echo "=========================================="
    echo "Ms Pacman: Recency Sampling with recexp=0.1 (5 seeds)"
    echo "  PID: $PID"
    echo "  Master log: $MASTER_LOG"
    echo "=========================================="
    echo ""
    echo "Experiments (5 total):"
    echo "  Recency sampling (recexp=0.1): seed 0-4"
    echo ""
    ...
    echo "  - replay.recexp: 0.1 (weaker decay)"
    echo ""
...
echo "=========================================="
echo "Starting Ms Pacman: Recency Sampling (recexp=0.1) experiments"
...
###############################################################################
# Recency実験 (recexp=0.1) - 5 seeds
###############################################################################
for SEED in 0 1 2 3 4; do
    EXP_COUNT=$((EXP_COUNT + 1))
    EXP_NAME="recency_exp01_seed${SEED}"
...
echo "Total: 5 experiments (recency sampling, recexp=0.1)"
echo "=========================================="
